...
- On the `Ubuntu WSL` terminal run the following to set up `Apptainer`.
...
- To build the Singularity Image File (SIF) for the `master`[^apptainer-win-wsl2-1] branch (`dynr-rocker.sif`) run the following:

[^apptainer-win-wsl2-1]: To build the Singularity Image File (SIF) for the `arma` branch (`dynr-arma-rocker.sif`), replace `dynr-master-sif.sh` with `dynr-arma-sif.sh`
...
- Type `exit` to exit the Apptainer shell and go back to `Ubuntu` WSL.
