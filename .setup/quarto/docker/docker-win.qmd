...
[^docker-win-2]: Note for developers only: Replace `jeksterslab/dynr-rocker` with `jeksterslab/dynr-arma-rocker` to use the `arma` branch.
...
-  Open http://localhost:8787 on your web browser to launch `Rstudio Server`.
    -  username: rstudio
    -  password: yourpassword[^docker-win-3]

[^docker-win-3]: You can change the password by replacing `yourpassword` in `PASSWORD=yourpassword`.
