...
dynr.mc <- function(object,
  ...
  mc <- MASS::mvrnorm(
    n = R,
    mu = coef(object)[def_vars],
    Sigma = vcov(object)[def_vars, def_vars]
  )
  # function to parse and execute the input def
  foo <- function(def) {
    mc_dist <- rep(x = NA, times = R)
    bar <- function(i) {
      i1 <- as.list(i)
      env <- list2env(i1)
      return(
